//
//  KeystoneBitcoinSDKTests.swift
//  
//
//  Created by LiYan on 3/30/23.
//

import Foundation
import XCTest
@testable import KeystoneSDK


final class KeystoneBitcoinSDKTests: XCTestCase {
    
    func testParsePsbt() {
        let psbtCborHex = ""
        
        let bitcoinSdk = KeystoneBitcoinSDK()
        let psbt = try! bitcoinSdk.parsePsbt(cborHex: psbtCborHex)
        
        XCTAssertEqual(psbt, "")
    }

    func testParsePsbtError() {
        let psbtCborHex = ""
        let bitcoinSdk = KeystoneBitcoinSDK()

        var thrownError: Swift.Error?
        XCTAssertThrowsError(try bitcoinSdk.parsePsbt(cborHex: psbtCborHex)) {
             thrownError = $0
        }
        XCTAssertEqual(thrownError as? KeystoneError, .parsePSBTError("PSBT is invalid"))
    }

    func testGeneratePsbtUR() {
        let psbtHex = ""

        let bitcoinSdk = KeystoneBitcoinSDK()
        let psbtEncoder = try! bitcoinSdk.generatePSBT(psbtHex: psbtHex)

        XCTAssertEqual(psbtEncoder.nextPart(), "ur:crypto-psbt/1-2/lpadaocsptcybkgdcarhhdgohdosjojkidjyzmadaenyaoaeaeaeaohdvsknclrejnpebncnrnmnjojofejzeojlkerdonspkpkkdkykfelokgprpyutkpaeaeaeaeaezmzmzmzmlslgaaditiwpihbkispkfgrkbdaslewdfycprtjsprsgksecdratkkhktimndacnch")
    }

    func testGeneratePsbtURError() {
        let psbtHex = ""
        let bitcoinSdk = KeystoneBitcoinSDK()

        var thrownError: Swift.Error?
        XCTAssertThrowsError(try bitcoinSdk.generatePSBT(psbtHex: psbtHex)) {
             thrownError = $0
        }
        XCTAssertEqual(thrownError as? KeystoneError, .generatePSBTError("PSBT is invalid"))
    }
}
